...
impl<T: Copy + std::ops::Sub<Output=T> + std::ops::Add<Output=T> + std::ops::Mul<Scalar, Output = T>> Bounds<T> {
    ...
    pub fn min(&self) -> &T { &self.0 }
    pub fn max(&self) -> &T { &self.1 }
    pub fn min_mut(&mut self) -> &mut T { &mut self.0 }
    pub fn max_mut(&mut self) -> &mut T { &mut self.1 }

    pub fn size(&self) -> T { *self.max() - *self.min() }
    pub fn center(&self) -> T { (*self.max() + *self.min()) * 0.5 }
}

pub type Bounds2D = Bounds<Vec2D>;

impl Bounds2D {
    pub fn square(a: f64, center: Vec2D) -> Self {
    ...
    pub fn scaled(&self, scale: f64) -> Bounds2D {
...
impl Default for Bounds2D {
    fn default() -> Self {
        Self(Vec2D::splat(Scalar::INFINITY), Vec2D::splat(Scalar::NEG_INFINITY))
    }
}
...
impl Bounds3D {
    ...
    pub fn cube(a: f64, center: &Vec3D) -> Self {
        let v = Vec3D::splat(a) * 0.5;
        Self(*center - v, *center + v)
    }

    pub fn centered_cube(a: f64) -> Self {
        let v = Vec3D::splat(a) * 0.5;
        Self(-v, v)
    }
    ...
    pub fn contains(&self, p: &Vec3D) -> bool {
        p.x >= self.0.x && p.y >= self.0.y && p.z >= self.0.z &&
        p.x < self.1.x && p.y < self.1.y && p.z < self.1.z
    }
}

impl Default for Bounds3D {
    fn default() -> Self {
        Self(Vec3D::splat(Scalar::INFINITY), Vec3D::splat(Scalar::NEG_INFINITY))
    }
}
...
pub fn adapt(v0: f64, v1: f64) -> f64 {
...
impl<T: Copy> Quad<T> {
    ...
    pub fn make_triangles(&self) -> (Triangle<T>, Triangle<T>) {
        (Triangle(self.2, self.1, self.0), Triangle(self.0, self.3, self.2))
    }    
}
